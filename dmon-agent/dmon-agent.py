...
class FetchStormLogsSDAll(Resource):
    def get(self):
        ...
        log_packers = {
            "0.9": self._packageLogs09,
            "1.0": self._packageLogs10,
        }
        ...
        if storm_version not in log_packers:
            ...
            response = jsonify({'Status': 'Not mplemented',
                                'Message': 'Unsupported storm version'})
            response.status_code = 501
            return response

        try:
            logs, success = log_packers[storm_version](stDir)
            return send_from_directory(*os.path.split(logs),
                                       as_attachement=True,
    ...
    def _packageLogs09(log_path):
        ...
        tarlog = self._ensureEmptyTarlog()
    ...
    @staticmethod
    def _packageLogs10(log_path):
        if not os.path.exists(os.path.join(log_path, 'workers-artifacts'))):
            raise ValueError('No Storm log files found')

        tarlog = self._ensureEmptyTarlog()
        with tarfile.open(tarlog, mode='w') as out:
            filename = os.path.split(log_path)[1]
            out.add(log_path, arcname=filename)
            app.logger.info(
